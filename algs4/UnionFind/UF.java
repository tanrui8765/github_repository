...
public class UF {
    ...
    public static void main(String[] args) {
        // 1. Step 1
        StdOut.println("Enter Main, please input UF numbers:");
        int N = StdIn.readInt();
        UF uf = new UF(N);

        // 2. Step 2
        StdOut.println("Created "+ N +" UF Sites");
        while (!StdIn.isEmpty()) {
            int p = StdIn.readInt();
            int q = StdIn.readInt();
            ...
            for(int i=0; i<N; i++)
            	StdOut.print(uf.parent[i]);	StdOut.println(" ");
            StdOut.println("\r");
            
            StdOut.println("Rank: ");	
            for(int i=0; i<N; i++)	
            	 StdOut.print(uf.rank[i]); StdOut.println(" ");
            StdOut.println("\r");
        }
